...
def play_notification_sound():
    ...
    RATE = 44100
    p = pyaudio.PyAudio()
    wf = wave.open(r"D:\nndt\NhamThanhDat_2100011239_NhanDangDoiTuong\NhanDangChay\baodong1.wav", 'rb')
    ...
    stream.close()
    p.terminate()
    last_notification_time = time.time()  # Ghi nhận thời điểm cuối cùng của âm thanh cảnh báo được phát

# Hàm để dừng phát âm thanh cảnh báo
...
def detect_and_alert(cap, model):
    ...
    if not is_fire_smoke_detected:
        stop_notification_sound()
        time.sleep(3)
# Khởi tạo video từ file chaydoan.mp4
cap = cv2.VideoCapture(r"D:\nndt\NhamThanhDat_2100011239_NhanDangDoiTuong\NhanDangChay\test_model.mp4")

# Khởi tạo mô hình YOLO
model = YOLO(r'D:\nndt\NhamThanhDat_2100011239_NhanDangDoiTuong\NhanDangChay\yolo.pt')
